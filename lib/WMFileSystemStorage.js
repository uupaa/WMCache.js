        ...
        dirReader["readEntries"](function(fileEntries) {
            if (fileEntries.length) {
                for (var i = 0, iz = fileEntries.length; i < iz; ++i) {
                    ...
                    var id = decodeURIComponent(fileEntry["name"]);

                    if (fileEntry["isFile"]) {
                        that._fileList[id] = 0; // add/update id
                    }
                }
                _list();
            } else {
    ...
    function _getFileSize() {
        ...
        function _getFile(fileEntry) {
            fileEntry["file"](function(file) {
                var id = decodeURIComponent(file["name"]);

                that._fileList[id] = file["size"];
                task["pass"]();
            }, missfn);
        }
    }
    ...
    this._dir["getFile"](fileName, { "create": true }, function(fileEntry) {
        fileEntry["createWriter"](function(fileWriter) {
            ...
            fileWriter["onwriteend"] = function() {
                if (code === 200) {
                    that._fileList[id] = size; // update snapshot
                } else { // code 0, code 413, code 503
                    delete that._fileList[id];
        ...
        var code = ERROR_CODES[error.name] || 503;
        delete that._fileList[id];
        callback(code);
    });
}
...
function WMFileSystemStorage_drop(id, callback) {
    ...
    this._dir["getFile"](fileName, { "create": false }, function(fileEntry) {
        delete that._fileList[id]; // update snapshot
