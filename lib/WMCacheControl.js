...
WMCacheControl["prototype"]["gc"]      = WMCacheControl_gc;      // WMCacheControl#gc(cache:WMCache, all:Boolean):void

// --- implements ------------------------------------------
function _normalize(url) {
    return url.replace(REL_PATH_NORMALIZE, "");
}
...
function WMCacheControl_gc(cache, // @arg WMCache
    ...
    var target = [];
    for (var key in list) {
        for (var pattern in this._garbage) {
            if (WMURL["match"](pattern, key)) {
                target.push(key);
            }
        }
    }
