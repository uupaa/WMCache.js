...
//}@dev
    ...
    function _ready() {
        var backend = that._storage instanceof FS ? "FileSystem"
                    : that._storage instanceof DB ? "IndexedDB" : "";
        callback(that, backend);
    }
}
...
//{@dev
    ...
    $valid($type(callback, "Function"),         WMCache_store, "callback");
//}@dev
    ...
    if (convertToArrayBuffer) {
        ...
        reader["onloadend"] = function() {
            that._storage["store"](key, reader["result"], mime, size, callback);
        };
        reader["readAsArrayBuffer"](data);
    } else {
        this._storage["store"](key, data, mime, size, callback);
    }
}
